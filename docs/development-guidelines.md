...
### 1.6 ドキュメンテーション
- 公開シンボル（`defaction` / `make-actions-app` / `actions-app` / `action-endpoint` / `*app*`）には docstring を付ける。
...
### 4.3 テストの隔離
- グローバル `*app*` に依存するテストは、`let` 再束縛または `make-actions-app` 再生成でテストごとに隔離する（NFR3）。副作用がテスト間に漏れないこと。
