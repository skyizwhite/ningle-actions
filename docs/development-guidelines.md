...
- 副作用のある処理（ルート登録・`*app*` 変更）は関数として切り出し、マクロ本体は薄く保つ。
...
- パッケージ名はファイルパスに一致させる（`src/app.lisp` → `ningle-actions/app`）。
...
### 1.5 エラーハンドリング
- ライブラリが定義・送出する状況（404 / 405）は **Clack 形式リスト**で返す。例外を濫用しない。
- アクション本体内のアプリ固有エラーは握りつぶさず上位（ningle / Lack）へ伝播させる（[functional-design.md](./functional-design.md) §6）。
- 入力検証は利用者責務。ライブラリ側で暗黙の検証・変換を行わない。

### 1.6 ドキュメンテーション
- 公開シンボル（`defaction` / `make-action-app` / `actions-app` / `action-endpoint` / `*app*`）には docstring を付ける。
...
| 種別 | 規約 | 例 |
|------|------|----|
| 関数・変数 | ケバブケース（小文字 + `-`） | `make-action-app` `dispatch-action` |
| 特殊変数 | `*earmuffs*` | `*app*` |
| 定数 | `+plus-signs+` | `+action-prefix+` |
| クラス | 通常はケバブケース | `actions-app` |
| 述語 | `-p` / `?` は付けず CL 慣習の `-p` | `action-exists-p` |
| マクロ | 関数と同様。定義系は `def-` 接頭辞可 | `defaction` |
| パッケージ | `ningle-actions/<file>` | `ningle-actions/app` |
...
- コア機構（F1〜F5）の主要パスを必ずカバーする:
  ...
  - `dispatch-action`: 正常呼び出し / 未登録 id → 404 / メソッド不一致 → 405。
  - `action-endpoint` が `/actions/<id>` を返す。再定義で `action_id` が再利用され URL が安定する。
  - `params` が ningle と同じ形でアクション本体へ渡る。
- HTTP レベル検証は `ningle:to-app` 済みアプリへテスト用 env を流して行う（mount は利用者責務のため、テストではアクションアプリ単体、または `:mount` 込みの builder の双方を検証）。

### 4.3 テストの隔離
- グローバル `*app*` に依存するテストは、`let` 再束縛または `make-action-app` 再生成でテストごとに隔離する（NFR3）。副作用がテスト間に漏れないこと。
